...
2. Build - 서브에이전트를 만들어 구현한다. 웹앱은 /app/{앱이름}/ 폴더에 독립적으로 만든다. 블로그의 다른 파일을 건드리지 않는다.
3. Review - 별도 서브에이전트를 만들어 검증한다. 브라우저에서 정상 작동하는지, 코드에 문제가 없는지 확인하고 review.md를 작성한다. 문제가 있으면 수정한다.
...
* 모바일에서도 사용할 수 있어야 한다.
* 모든 웹앱은 블로그의 색상 팔레트를 따른다. css/style.css의 CSS 변수(--bg, --bg-secondary, --text, --text-secondary, --accent, --accent-hover, --border 등)를 참조할 것.
